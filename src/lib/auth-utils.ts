...
//             nookies.set({ res }, _storageKeys.token, response.token, {
//               maxAge: 1 * 24 * 60 * 60, //1 day
//               path: "/",
//               httpOnly: false,
//             });

//             // Return the nested user object with token, properly typed for NextAuth
//             return {
//               id: response.user.name,
//               name: response.user.name,
//               email: response.user.email,
//               image: response.user.profile_picture || null,
...
//           token.token = token;
//           token.expires = new Date(Date.now() + 60 * 60 * 1000).toISOString();
//         }
//         return token;
//       },
